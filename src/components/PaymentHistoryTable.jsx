...
import PaymentList from "../../helper/PaymentList";
...
const InvoiceDownloadBtnStyles = styled.button`
   display: flex;
   align-items: center;
   gap: 9px;   
`;

const InvoiceDownloadBtn = (record) => {
   return (
      <InvoiceDownloadBtnStyles>
         <DownloadIcon />
         <p>PDF</p>
      </InvoiceDownloadBtnStyles>
   );
...
const ApplicationNoBodyStyles =  styled.p`
   
`

const ApplicationNoBody = (record) => {
   return <ApplicationNoBodyStyles>#{record.id}</ApplicationNoBodyStyles>;
};

const PaymentHistoryTable = () => {
   const [record, setRecord] = useState(PaymentList);
   ...
   return (
      <DatatableStyles>
         ...
         <DataTable value={record} size="large" paginator rows={8} rowsPerPageOptions={[5, 8, 10, 20, 50]} rowHover dataKey="id" header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} tableStyle={{ minWidth: "700px", maxWidth: "948px" }}>
            {/* <Column selectionMode="multiple" exportable={false} headerStyle={{ width: "3rem" }}></Column> */}
            <Column className="tableText dark-text" style={{ textAlign: "left" }} headerStyle={{ width: "10rem" }} field="id" body={ApplicationNoBody} header="Application No" />
            <Column className="tableText " headerStyle={{ width: "116px", padding: 0 }} style={{ padding: 0 }} field="date" header="Payment Date" />
            <Column className="tableText admission-amount" style={{ textAlign: "center" }} headerStyle={{ width: "190px" }} field="admissionAmount" header="Admission Amount" />
            <Column className="tableText" field="location" header="Invoice" body={InvoiceDownloadBtn} />
            <Column className="tableText" field="status" header="Status" body={StatusBody} />
            <Column className="tableText" field="statusDate" headerStyle={{ textAlign: "center" }} header={"Status Date"} />
         </DataTable>
      </DatatableStyles>
   );
};
