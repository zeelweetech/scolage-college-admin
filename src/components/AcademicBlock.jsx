...
const AcademicBlock = ({ info }) => {
   ...
   const handleFormSubmit = async (e) => {
      ...
      try {
         ...
         if (newAcademicList.length) {
            ...
            const { data: newList } = await axios.post('/v2/reg/acedemicdetail', newFormData, {
               headers: {
                  "Content-Type": "multipart/form-data",
               }
            })
   ...
   const handleAddField = (e) => {
      e.preventDefault();
      if (editable) {
         setNewAcademicList([...newAcademicList, { image: "", more_info: "", imgPreview: "" }]);
      } else {
         toast.dismiss();
         toast.error("Edit Details are not Allowed !!");
      }
                  ...
                  <GalleryStyle key={i}>
                     <div className="image-block">
                        ...
                        <div className="image-btn">
                           ...
                           <input type="file" style={{ display: "none" }} accept="image/*" id={`academic-new-upload-${i}`} name={"image"} onChange={(e) => handleNewChange(e, i)} />
                        </div>
                        <div className="remove-btn">
                           <button
                              type="button"
