...
const CulturalBlock = ({ info }) => {
   ...
   const handleFormSubmit = async (e) => {
      ...
      try {
         ...
         if (newCulturalList.length) {
            ...
            const { data: newList } = await axios.post('/v2/reg/culturaldetail', newFormData, {
               headers: {
                  "Content-Type": "multipart/form-data",
               }
            })
   ...
   const handleChange = (e, i) => {
      if (editable) {
         ...
         if (name == "image") {
            ...
            const fileName = `cultural\$${uuidv4()}\$${localStorage.getItem("collegeProfileId")}.${fileExt}`;
   ...
   const handleAddField = (e) => {
      e.preventDefault();
      if (editable) {
         setNewCulturalList([...newCulturalList, { image: "", more_info: "", imgPreview: "" }]);
      } else {
         toast.dismiss();
         toast.error("Edit Details are not Allowed !!");
      }
                  ...
                  <GalleryStyle key={i}>
                     <div className="image-block">
                        ...
                        <div className="image-btn">
                           ...
                           <input type="file" style={{ display: "none" }} accept="image/*" id={`cultural-new-upload-${i}`} name={"image"} onChange={(e) => handleNewChange(e, i)} />
                        </div>
                        <div className="remove-btn">
                           <button
                              type="button"
                     ...
                     <div className="info-block">
                        <textarea placeholder="More Info" value={item.more_info} name="more_info" onChange={(e) => handleNewChange(e, i)}></textarea>
                     </div>
                  </GalleryStyle>
               ))}
