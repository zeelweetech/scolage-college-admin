...
const SportUploadBlock = ({ info }) => {
   ...
   const handleFormSubmit = async (e) => {
      ...
      try {
         ...
         if (newSportsList.length) {
            ...
            const { data: newList } = await axios.post("/v2/reg/sportsdetail", newFormData, {
               headers: {
                  "Content-Type": "multipart/form-data",
               },
            });
   ...
   const handleChange = (e, i) => {
      if (editable) {
         ...
         if (name == "image") {
            ...
            const fileName = `sports\$${uuidv4()}\$${localStorage.getItem("collegeProfileId")}.${fileExt}`;
   ...
   const handleAddField = (e) => {
      e.preventDefault();
      if (editable) {
         setNewSportsList([...newSportsList, { image: "", more_info: "", imgPreview: "" }]);
      } else {
         toast.dismiss();
         toast.error("Edit Details are not Allowed !!");
      }
                  ...
                  <GalleryStyle key={i}>
                     <div className="image-block">
                        ...
                        <div className="image-btn">
                           ...
                           <input type="file" style={{ display: "none" }} accept="image/*" id={`sport-new-upload-${i}`} name={"image"} onChange={(e) => handleNewChange(e, i)} />
                        </div>
                        <div className="remove-btn">
                           <button
                              type="button"
