...
const InfrastructureForm = ({ info }) => {
   ...
   const handleFormSubmit = async () => {
      ...
      try {
         const {data} = await axios.put(`/v2/edit/infrastr/${formData.infraid}`,formData)
         setEditable(false)
            ...
            <div className="infra-left">
               <div className="infra-radios-main">
                  {CheckData.map((item, index) => (
                     <InfraChecks disabled={!editable} key={index} name="infra-checks" data={item} infraChecks={infraChecks} formData={formData} handleCheckChange={handleCheckChange} setInfraChecks={setInfraChecks} />
                  ))}
               </div>
            </div>
            <div className="infra-right">
               <MoreInfoField height={"500px"} name="moreinfo" value={formData?.moreinfo} handleChange={handleChange} />
            </div>
         </Wrapper>
