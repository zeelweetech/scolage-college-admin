...
const ToppersBlock = ({ info }) => {
   ...
   const handleFormSubmit = async (e) => {
      ...
      try {
         ...
         if (topperInfoList.length) {
            ...
            const { data } = await axios.patch("/v2/alutoppers/edit/all/array", formData, {
               headers: {
         ...
         if (newTopperInfoList.length) {
            ...
            const { data: newList } = await axios.post('/v2/reg/alutoppers', newFormData, {
               headers: {
                  "Content-Type": "multipart/form-data",
               }
            }) 
   ...
   const handleChange = (e, i) => {
      if (editable) {
         ...
         if (name == "image") {
            ...
            const fileName = `toppers\$${uuidv4()}\$${localStorage.getItem("collegeProfileId")}.${fileExt}`;
   ...
   const handleAddFields = (e) => {
      e.preventDefault();
      if (editable) {
         setNewTopperInfoList([...newTopperInfoList, { image: "", name: "", passYear: "", marks: "", moreInfo: "", imagePreview: "" }]);
      } else {
         toast.dismiss();
         toast.error("Edit details is not allowed !!");
      }
                     ...
                     <div className="topper-item-top">
                        <div className="image-block">
                           ...
                           <div className="image-btn">
                              ...
                              <input type="file" accept="image/*" id={`file-new-upload-${i}`} name="image" onChange={(e) => handleNewChange(e, i)} style={{ display: "none" }} />
                           </div>
                        </div>

                        <div className="topper-info-block">
                           <input type="text" placeholder="Name" name="name" value={item.name} onChange={(e) => handleNewChange(e, i)} />
                           <input type="number" placeholder="Passing out year" name="passYear" value={item.passYear} onChange={(e) => handleNewChange(e, i)} />
                           <input type="number" placeholder="Marks" name="marks" value={item.marks} onChange={(e) => handleNewChange(e, i)} />
                        </div>
                     </div>

                     <div className="more-info-block">
                        <textarea placeholder="More Info" name="moreInfo" value={item.moreInfo} onChange={(e) => handleNewChange(e, i)}></textarea>
                     </div>
                  </TopperItemStyles>
               ))}
