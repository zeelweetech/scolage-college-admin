...
const CollegeImagesBlock = ({ info }) => {
   ...
   const handleFormSubmit = async (e) => {
      ...
      try {
         ...
         if (collegeImage.length) {
            ...
            const { data } = await axios.patch("/v2/clgimage/edit/all/edit", formData, {
               headers: {
         ...
         if (newCollegeImage.length) {
            ...
            const { data: newList } = await axios.post("/v2/reg/clgimage", newFormData, {
               headers: {
                  "Content-Type": "multipart/form-data",
               },
            });
   ...
   const handleAddField = (e) => {
      e.preventDefault();
      if (editable) {
         setNewCollegeImage([...newCollegeImage, { image: null, imagePreview: null, title: "" }]);
      } else {
         toast.dismiss();
               ...
               {newCollegeImage.map((item, i) => (
                  <ImageItemStyle key={i}>
                     <div className="clg-image-block">
                        <div className="clg-img">{item.imagePreview ? <img src={item.imagePreview} alt="" /> : <p>Add Image*</p>}</div>
                        <div className="img-ctas">
                           <label className="edit-cta" htmlFor={`college-new-image-${i}`}>
                              <AddIconBig />
                              <p>{item.image ? "Edit" : "Add"}</p>
                           </label>
                           <input type="file" accept="image/*" required name="image" id={`college-new-image-${i}`} onChange={(e) => handleNewChange(e, i)} style={{ display: "none" }} />
                           {/* {item.image && (
                              <button type="button" className="remove-cta" onClick={(e) => handleRemoveImage(e, i)}>
                                 <RemoveIcon />
                                 <p>Remove</p>
                     ...
                     <div className="clg-img-title">
                        <input type="text" required name="title" value={item.title} onChange={(e) => handleNewChange(e, i)} placeholder="Name" />
                     </div>
                  </ImageItemStyle>
               ))}
